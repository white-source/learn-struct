...
path='./radar_train3/'
#模型保存地址
model_path='./radar_net3/model.ckpt'
...
def read_img(path):
    ...
    for idx,folder in enumerate(cate):
        for im in glob.glob(folder+'/*.png'):
            ...
            img=io.imread(im)
            img=transform.resize(img,(width,heigh),mode="constant")
            imgs.append(img)
            labels.append(idx)
...
#-----------------构建网络----------------------
#占位符
x=tf.placeholder(tf.float32,shape=[None,width,heigh,rgb],name='x')
...
for epoch in range(n_epoch):
    ...
    for x_train_a, y_train_a in minibatches(x_train, y_train, batch_size, shuffle=True):
        ...
        print("局部准确率:"+str(np.sum(ac)))
    print("训练成功率:"+str(epoch)+str((np.sum(train_acc)/ n_batch)))
