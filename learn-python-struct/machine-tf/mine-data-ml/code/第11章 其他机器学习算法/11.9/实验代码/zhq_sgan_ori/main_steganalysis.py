...
flags = tf.app.flags
flags.DEFINE_string('model_name', 'stego', 'Name of trainable model')
...
flags.DEFINE_string("checkpoint_dir", "checkpoint/exam", "Directory name to save the checkpoints [checkpoint]")
flags.DEFINE_string("sample_dir", "samples", "Directory name to save the image samples [samples]")
flags.DEFINE_boolean("is_train", True, "True for training, False for testing [False]")  #False
...
flags.DEFINE_string('data', '/home/ist/zhq/SGAN_adversarial-steganography-master/data/', 'Dataset directory')
#'/home/dvolkhonskiy/SGAN/code/data/overtraining/',
flags.DEFINE_string('dataset_name', 'celebA', 'Dataset Name')  #origin : celeba
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        dirs = ['/home/ist/zhq/SGAN_adversarial-steganography-master/data/overtraining_ori/plus_1']
#�Ѹ�
        dirs_test = ['/home/ist/zhq/SGAN_adversarial-steganography-master/data/overtraining/test_%s'% i for i in range(0, 10)]  #plus_%s' % i for i in range(11, 17)

        if FLAGS.is_train:
            steganalisys.train(counter=1, gen_dirs=dirs)
        else:
            # steganalisys.load(FLAGS.checkpoint_dir, step=28481) # LSB matching clf
            steganalisys.load(FLAGS.checkpoint_dir, step=97)   # step=59

            for gen_dir in dirs_test:
