...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    self.loss_multiple = tf.placeholder(tf.float32, shape=[config.batch_size, self.num_steps], name="loss_multiple")
    loss = tf.multiply(loss, self.loss_multiple)
...
def run_epoch(session, model, eval_op=None, verbose=False, composing=False):
  ...
  for step in range(composing and 1000 or model.input.epoch_size):
    ...
    loss_multiple = np.copy(target_data)
    for x in np.nditer(loss_multiple, op_flags=['readwrite']):
      x[...] = x<3 and 1000 or 1
    feed_dict[model.loss_multiple] = loss_multiple
    ...
    if composing:
      indices, words = resolve_logits(logits[0][0])
      print("%s -->%s"%(words_by_id[feed_dict[model.compose_input][0][0]], ",".join(words),))
...
def main(_):
  ...
  train_data, valid_data, test_data, _, words_by_id, word_to_id = raw_data
  ...
  # with tf.Graph().as_default():
    ...
    with sv.managed_session(config=config_proto) as session:
      if FLAGS.compose:
        test_perplexity = run_epoch(session, mtest, composing=True)
        return
      ...
      for i in range(config.max_max_epoch):
        ...
        m.assign_lr(session, max(config.learning_rate * lr_decay, 0.001))
