...
blobs = datasets.make_blobs(n_samples=n_samples//3, random_state=8, n_features=2, centers=[[3.8, 0.9]] ,cluster_std=[[0.13]])
no_structure = (np.random.rand(200, 2)+[-0.2, -0.3])*[7, 2], None
...
plt.subplot(132)
dbscan = DBSCAN(eps=0.15, min_samples=10).fit(X)
...
for k, col in zip(unique_labels, colors):
    if k == -1:
        plt.scatter(X[:, :1][labels==k], X[:, 1:][labels==k], color='k', s=5)
    else:
        plt.scatter(X[:, :1][labels==k], X[:, 1:][labels==k], color=col, s=20)
plt.xticks(())
plt.yticks(())
